...
function SignInGate({ onBack }) {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      // Page redirects to Google then back to /admin — no need to setBusy(false)
    } catch (err) {
      console.error(err)
      setError(
        err?.message?.includes('provider is not enabled')
          ? 'Google provider مش مفعّل في Supabase. روح: Supabase Dashboard → Auth → Providers → Google → Enable'
          : err?.message || 'فشل تسجيل الدخول. حاول تاني.'
      )
      setBusy(false)
    }
  }
